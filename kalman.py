...
class Kalman:
    ...
    def update(self, landmark_mask, landmark_pos):
        ...
        try:
            ...
            print(np.hstack((H.dot(self.x), z)))
            #import sys
            #sys.exit(0)
            return
               
        K = self.P.dot(H.T).dot(invS)
...
if __name__ == '__main__':
    ...
    landmarks = np.vstack((
        np.random.uniform(0.0, 100.0, [1, 10]),
        np.random.uniform(0.0, 20.0, [1, 10])))

    # Virtual x,y,phi robot
    robot = XYRobot(pose=[50,10,0], err=0.0) 
    world['robot'] = robot  

    # Virtual sensor reporting bearings in robot space. Detectable landmarks are limited by FOV, max-dist and obstacles
    sensor = LandmarkSensor(landmarks, err=0.0, fov=2.0 * math.pi, maxdist=100., measure='bearing')
    ...
    P[2, 2] = 0.0
    P[3, 3] = 0.0
    k = Kalman(true_state, true_state, P)
